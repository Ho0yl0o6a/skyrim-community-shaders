...
namespace MOC
{
	...
	namespace
	{
		...
		// Our own frame index; see MOC::BeginFrame for why the engine's is not usable here.
		...
		// The engine's frozen CULL camera: the NiCamera object that DrawWorld hands to every
		// MAIN-scene cull (BuildSceneLists cell culls, MainAccum subtree culls, RenderDepth,
		// RenderWorld). Distinct from the render camera -- its NiFrustum is normalized to
		// (-1,1,1,-1), so it identifies the pass but can NOT provide the projection.
		//
		// The SE port read it from a global (REL::ID 528062) that has no AE counterpart, so it
		// is identified by that frustum signature instead and latched on first sight. No camera
		// that actually projects anything carries it. If the latch never happens the only
		// consequence is that fewer passes are tested -- it cannot cull something visible.
		std::atomic<RE::NiCamera*> g_cullCamera{ nullptr };

		bool HasNormalizedCullFrustum(RE::NiCamera* a_camera)
		{
			const auto& f = a_camera->GetRuntimeData2().viewFrustum;
			return f.fLeft == -1.0f && f.fRight == 1.0f && f.fTop == 1.0f && f.fBottom == -1.0f;
		}

		/** @brief Latches the frozen cull camera the first time one is seen. */
		void NoteCullCamera(RE::NiCamera* a_camera)
		{
			if (!a_camera || g_cullCamera.load(std::memory_order_relaxed))
				return;

			if (HasNormalizedCullFrustum(a_camera)) {
				g_cullCamera.store(a_camera, std::memory_order_relaxed);
				logger::info("[OcclusionCulling] latched the frozen cull camera by frustum signature");
			}
		}
		...
		LandOccluderMesh BuildLandMesh(const RE::TESObjectLAND::LoadedLandData* a_data, float a_worldX, float a_worldY)
		{
			...
			// heights[] are CELL-LOCAL Z relative to the cell's height midpoint, which the
			// engine's own consumers add back (TESObjectLAND::GetNiPointHeight). The SE port
			// read that midpoint from LoadedLandData+0x49C0, an offset CommonLibSSE-NG only
			// knows as `unk49C0`; heightExtents is the named (min, max) pair for the same cell,
			// so the midpoint comes from there instead. Reading the unnamed field put the land
			// occluders at the wrong height and they swallowed the mountains behind the town.
			const float zBase = 0.5f * (a_data->heightExtents.x + a_data->heightExtents.y);
			for (int q = 0; q < 4; ++q) {
		...
		{
			RE::NiNode* scene = RE::Main::WorldRootNode();  // "WorldRoot Node" (SceneGraph)
	...
	{
		return g_cullCamera.load(std::memory_order_relaxed);
	}
	...
	bool BuildOccluders(RE::NiCamera* a_camera)
	{
		...
		// depth-prepass / world subtree culls get the frozen CULL camera (identified by its
		...
		NoteCullCamera(a_camera);

		if (a_camera != renderCam && a_camera != g_cullCamera.load(std::memory_order_relaxed))
			return false;
		...
		// Sun shadow-gather camera. The SE port reached it through a scene-node global
		// (REL::ID 513211, no AE counterpart -- resolving it popped CommonLibSSE's modal
		// failure box on the render thread) and two raw offsets. Both offsets are named
		// members: ShadowSceneNode+0x210 is sunShadowDirLight and BSShadowDirectionalLight
		// +0x578 is fullFrustumCamera, so the typed path is both correct on every runtime and
		// self-describing. Read per frame; feeds the builder's sun pass and the sun-view
		// caster tests.
	...
	void BeginFrame()
	{
		g_frameIndex.fetch_add(1u, std::memory_order_relaxed);
	}
	...
	void DumpDebugImages()
	{
		...
		if (!s_dump || !g_init || !g_moc)
			return;
		if ((g_frameIndex.load(std::memory_order_relaxed) % 120u) != 0u)
			return;
	...
	bool TestObject(RE::NiAVObject* a_object)
	{
		...
		{
			if (!WaitForRasterComplete(g_frameIndex.load(std::memory_order_relaxed)))
	...
	{
		return a_camera && (a_camera == GetMainCamera() || a_camera == g_cullCamera.load(std::memory_order_relaxed));
	}
	...
	bool TestObjectSunView(RE::NiAVObject* a_object)
	{
		...
		{
			if (g_sunRasterFrame.load(std::memory_order_acquire) != g_frameIndex.load(std::memory_order_relaxed))
				return true;
		}
		const auto& b = a_object->worldBound;
		if (b.radius <= 0.0f)
