...
namespace MOC
{
	 ...
	 *
	 * Must be called exactly once per rendered frame. The SE port keyed the per-frame build
	 * claim on BSGraphics::State::frameCount, which does not advance on this runtime: the
	 * claim then succeeded once, every later frame saw the same index and took the
	 * already-claimed path, and every object was tested for the rest of the session against
	 * the occluder buffer rasterized on the first frame. That reads as a large speed-up and is
	 * really just culling against a stale depth buffer. Owning the counter removes the
	 * dependency entirely.
	 */
	void BeginFrame();
